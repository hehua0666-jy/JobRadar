...
- 具体岗位与项目级入口区分
- 本周新增、临近截止、未投递等工作流筛选
- 新秋招公司雷达：仅展示最新发现批次，并跟踪核验与本地已投状态
- 历史招聘事件：默认折叠展示早于最新批次的公司级招聘事件
...
当前岗位与招聘事件的“已投递”状态分别保存在浏览器 `localStorage` 中：
- 其他访客看不到你的投递状态
- 其他访客的操作不会影响你
- 但不同设备之间暂时不会自动同步
...
- 岗位变化数据继续由后台工具和 GitHub Actions 维护；v1.3 起不再在首页展示变化雷达
...
- “新秋招公司雷达”按最大的有效 `discovered_at` 自动识别最新发现批次。
- 较早招聘事件进入默认折叠的紧凑历史区域。
- 招聘事件使用独立的 `jobradarAppliedRecruitmentLeads` 本地已投状态，不与正式岗位混用。
- 移除“岗位变化雷达”前端模块，保留后台差异数据、工具与工作流。
